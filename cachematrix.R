...
##   an error is returned if m is not a square matrix (since only square matrices can be inverted)
...
makeCacheMatrix <- function(x = matrix()) {
        ...
        ## if a value is provided for x, check that it is a matrix indeed
        if (!is.null(x)) {
...
##  relying on R to stop with various messages when m has not been created with makeCacheMatrix
...
cacheSolve <- function(x, ...) {
        ...
        ##   hence so no parameter type checking
        ...
        ##  note that x is assumed to be invertiblen, hence no explicit error handling
